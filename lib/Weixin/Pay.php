...
class PayManager {
	 ...
	 */
	public function getPackageForNativeUrl($body, $attach, $out_trade_no, $total_fee, $notify_url, $spbill_create_ip, $time_start, $time_expire, $transport_fee, $product_fee, $goods_tag, $noncestr, $bank_type = "WX", $fee_type = 1, $input_charset = "GBK", $retcode = 0, $reterrmsg = "ok") {
	...
	/*
	 * 获取JS API 时所需的订单详情（package） 在商户调起JS API 时，商户需要此时确定该笔订单详情，并将该订单详情通过一定的
	 * 方式进行组合放入package。JS API 调用后，微信将通过package 的内容生成预支付单。下 面将定义package
	 * 的所需字段列表以及签名方法。
	 */
	protected function getPackage4JsPay($body, $attach, $out_trade_no, $total_fee, $notify_url, $spbill_create_ip, $time_start, $time_expire, $transport_fee, $product_fee, $goods_tag, $bank_type = "WX", $fee_type = 1, $input_charset = "GBK") {
		// package 所需字段列表
		// 参数 必填 说明
		...
		// 该时间取自商户服务器。取值范围：14 字节。
		// transport_fee 否 物流费用，单位为分。如果有值，必须保证transport_fee
		// +product_fee=total_fee。
		// product_fee 否 商品费用，单位为分。如果有值，必须保证transport_fee
		// +product_fee=total_fee。
